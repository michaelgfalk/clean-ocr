...
def split_sentences(x_data, y_data, tolerance = 10, adjust = 10., start = 'स', end = 'ए', max_len = 200):
  """
  ...
  for x, y in zip(x_data, y_data):
    ...
    while len(y) > 0:
      ...
      if y_stop_mtch:
        ...
        # The span method of the returned match object give the start and end idx of the match
...
def bucket_fun(x, y, max_batch_size, tolerance = 1.2):
  ...
  Returns:
  ==========
  bucketor (fun): bucketing function, instance of tf.data.experimental.bucket_by_sequence_length"""
...
def load_dataset(x_path = None, y_path = None, batch_size = 128, tolerance = 1.2, drop = False, test_size = .2, seqs = None, tkzr = None, max_len = None):
  """Load the dataset either from a saved tuple or from the raw text"""
  
  if seqs is not None and tkzr is not None:
    # Unpack saved data
    x_train, x_test, y_train, y_test, idx_train, idx_test = seqs
    tkzr = tkzr
  else:
    # Or load from raw text files
    x, y = import_raw_data(x_path, y_path)
    x, y, idxs = split_sentences(x, y, max_len = max_len)
    x, y, tkzr = tokenize(x, y)
    seqs = (x_train, x_test, y_train, y_test, idx_train, idx_test) = train_test_split(x, y, idxs, test_size = test_size)
  
  # Convert training data to dataset and bucket
  output_dtypes = (tf.int32, tf.int32)
  output_shapes = (tf.TensorShape([None]), tf.TensorShape([None]))
  
  train_gen = lambda: ((x,y) for x,y in zip(x_train, y_train))
  train_data = tf.data.Dataset.from_generator(train_gen, output_dtypes, output_shapes)
  
  train_bucketor, train_bnds, train_btchs = bucket_fun(x_train, y_train, batch_size, tolerance)
  ...
  # Same for test
  test_gen = lambda: ((x,y) for x,y in zip(x_test, y_test))
  test_data = tf.data.Dataset.from_generator(test_gen, output_dtypes, output_shapes)
  
  test_bucketor, test_bnds, test_btchs = bucket_fun(x_test, y_test, batch_size, tolerance)

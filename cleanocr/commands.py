...
@cli.command()
@click.option('-j', '--json', help='path to json configuration file, defaults to file included in package', type=str, default='use_included')
...
@click.option('-o', '--optimizer', help='optimizer to use', type=str)
@click.option('-b', '--max_batch_size', help='maximum size of training batches', type=int)
@click.option('-t', '--tolerance', help='tolerance parameter for sentence splitter', type=float)
@click.option('-l', '--max_len', help='maximum allowed length of training sequences', type=int)
@click.option('-e', '--epochs', help='number of epochs to train for', type=int)
@click.option('-nl', '--norm_lim', help='norm limit for gradient clipping', type=int)
...
@click.option('-nc', '--num_chars', help='number of characters in vocabulary; if not supplied, it is inferred from the data', type=int)
...
@click.option('-d', '--data_dir', help='directory where data is stored', type=str)
